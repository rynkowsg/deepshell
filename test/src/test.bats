...
test_composed_as_expected_relative() {
  ...
  (cd "${REPO_DIR}/test/res/test_suite/${name}" || exit 1; "${REPO_DIR}/main/src/pl/rynkowski/shellpack.cljc" pack -i "${input_path}" -o "${output_path}")
...
test_composed_as_expected_absolute() {
  ...
  "${REPO_DIR}/main/src/pl/rynkowski/shellpack.cljc" pack -i "${input_path}" -o "${output_path}"
  local output_value expected_value
...
test_compare_results_absolute() {
  local name=$1
  "${REPO_DIR}/main/src/pl/rynkowski/shellpack.cljc" pack -i "${REPO_DIR}/test/res/test_suite/${name}/entry.bash" -o "${REPO_DIR}/test/res/test_suite/${name}/output.bash"
  local res_entry res_bundled res_entry_status res_bundled_status
  res_entry="$(cd "${REPO_DIR}/test/res/test_suite/${name}" || exit 1; bash "entry.bash")"
  res_entry_status=$?
  res_bundled="$(cd "${REPO_DIR}/test/res/test_suite/${name}" || exit 1; bash "output.bash")"
